// 将mssql的字段类型转成mysql的字段类型
export const ToMysqlType = (columnType: string) => {
    switch (columnType) {
...
// 对应的neo4j中的类型，由于使用node导致只有string和int 类型。。。。。。。
export const ToNodeType = (columnType: string) => {
    switch (columnType) {
...
// 对应的neo4j中的类型，由于使用node导致只有string和int 类型。。。。。。。
export const ToNeoType = (columnType: string) => {
    switch (columnType) {
        case 'bigint': return 'string';
        case 'bit': return 'int';
        case 'decimal': return 'string';
